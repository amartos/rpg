...
static double round_angle(double const n)
{
...
static double sind(double const angle)
{
    return sin(radians(angle));
}

// cos(x) takes x as radians, so cosd(x) takes x as degrees
static double cosd(double const angle)
{
...
static void calc_formation_offset(Movement *movement, int const char_number)
{
    ...
    double ox = 0.0, oy = 0.0, angle = 0.0;
    ...
    if (char_number > 0)
    {
        ...
        switch(movement->formation)
        {
            // Line: 3210
            case LINE:
                angle = 270.0 + (double)(45 * direction); // direction is uint
                ox = round_angle(cosd(angle)) * -1 * char_number;
                oy = round_angle(sind(angle)) * -1 * char_number;
                break;
            /* square:
             * 01
             * 32 */
            case SQUARE:
                angle = 270.0 + (double)(45*char_number - 45) + (double)(45*direction); // direction is uint
                ox = round_angle(cosd(angle));
                oy = round_angle(sind(angle));
                break;
            /* "triangle":
             * 2
             *  01
             * 3 */
            case TRIANGLE:
                angle = 270.0 - half_cn * 135.0 - third_cn * 90.0 + (double)(45*direction); // direction is uint
                ox = round_angle(cosd(angle));
                oy = round_angle(sind(angle));
                break;
             ...
             * 3 */
            case CIRCLE:
                angle = 270.0 + (double)(45*char_number) + (double)(45*direction); // direction is uint
                ox = round_angle(cosd(angle)) + 1 + third_cn;
                oy = round_angle(sind(angle));
                break;
            default:
                break;
