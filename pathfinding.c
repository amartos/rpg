...
static Bool is_in_queue(int const max_array, Coord queue[max_array], Coord const coord)
{
...
static void set_in_queue(int const max_array, Coord queue[max_array], Coord const coord)
{
...
static Bool is_queue_empty(int const max_array, Coord const queue[max_array])
{
    for (int i=0;i<max_array;i++)
        if (!is_coord_empty(queue[i]))
            return FALSE;
        ...
        )
{
    ...
    int ipivot = max_array/2;
    coord_to_int(queue[ipivot], &x, &y);
    int pivot_cost = cost[y][x];
...
static void get_neighbours(Coord next[8], Coord const current, Map const map)
{
    ...
    next[N].x = current.x;
    next[N].y = current.y > 0.0 ? current.y - 1.0 : current.y;

    next[NE].x = current.x < map.max.x - 1.0 ? current.x + 1.0 : current.x;
    next[NE].y = current.y > 0.0 ? current.y - 1.0 : current.y;

    next[E].x = current.x < map.max.x - 1.0 ?  current.x + 1.0 : current.x;
    next[E].y = current.y;

    next[SE].x = current.x < map.max.x - 1.0 ? current.x + 1.0 : current.x;
    next[SE].y = current.y < map.max.y - 1.0 ? current.y + 1.0 : current.y;

    next[S].x = current.x;
    next[S].y = current.y < map.max.y - 1.0 ? current.y + 1.0 : current.y;

    next[SW].x = current.x > 0.0 ? current.x - 1.0 : current.x;
    next[SW].y = current.y < map.max.y - 1.0 ? current.y + 1.0 : current.y;

    next[W].x = current.x > 0.0 ? current.x - 1.0 : current.x;
    next[W].y = current.y;

    next[NW].x = current.x > 0.0 ? current.x - 1.0 : current.x;
    next[NW].y = current.y > 0.0 ? current.y - 1.0 : current.y;
}
...
{
    int x, y; coord_to_int(next_coord, &x, &y);
    int distance = a_star ? (int)(fabs(goal.x - next_coord.x) + fabs(goal.y - next_coord.y)) : 0;
    int next_cost = 1 + current_cost + map.cost[y][x] + distance;
    return next_cost;
}
        ...
        )
{
     ...
     * the array. */
    int max_array = 10 + (int)(map.max.x * map.max.y/10);
    ...
    int maxy = (int)map.max.y, cost[maxy][maxx];
    for (y=0;y<maxy;y++)
        for (x=0;x<maxx;x++)
            cost[y][x] = 0;

    int currx, curry; coord_to_int(start, &currx, &curry);
    ...
    while (TRUE)
    {
        ...
        else if (!is_coord_empty(current))
        {
            ...
            for (i=N;i<=NW;i++)
                ...
                {
                    coord_to_int(next[i], &nextx, &nexty);
